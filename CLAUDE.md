...
Hale is **the activity finder that never lets you miss it** — a number parents text to find what is on for their kids near them, and to be told in time to get a spot. It is not an assistant, and it does not describe itself as one (decision D22): a parent texts once, Hale finds activities that fit the child and the postal code, watches the sign-up mornings and waitlists that fill by 7:02, hands over the registration page when it opens, notices from a connected mailbox or calendar that they actually signed up, asks afterwards how it went, and passes what they say to the next parent who asks. The same finder follows the family to another city when a booking email shows they are travelling. The word *assistant* is banned from parent-facing copy, and so is *AI* as a selling point: the model, the Gmail and Calendar connectors, the co-parent seat and the evening check-in are how the finding works, never what is sold. Hale lives in SMS and email — parents should never need another app — while the web app is the receipts surface (approvals, audit, settings) rather than the daily one.

The F14 decision register fixes the choices that rest on this: messaging-first surface, single-ask intake then just-in-time everything, the registration-and-coverage radar as the flagship job, scoped caregiver roles, privacy as the moat, natural-language consent, ICS-first calendar output, the dashboard demoted to receipts, a dark launch released by one flag flip, and D22–D25 — the positioning above, asking only about what Hale saw, reviews and attendance as byproducts rather than a rating product, and never-template meaning voice rather than generation. The [F14 · Messaging-First Operator](https://linear.app/villagehale/project/f14-messaging-first-operator-hale-is-a-number-you-text-ad65e88d5775) Linear project — its design documents, that register, and the per-feature briefs — is the design source of truth; read it before changing product behaviour. Treat both the pre-pivot newborn-platform spec and the earlier "family chief of staff" framing as history rather than direction. (Compliance baseline is Canada — PIPEDA/Law 25 + data residency per hard rule #1; broadening to other regions is a deliberate multi-region decision, not an assumption.)
