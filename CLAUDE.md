...
Hale is a **planner for your kids' year** (**Find what's on. Hear how it went.**) — an event-driven, multi-agent quiet operator for families raising children across every stage of childhood (0–18; beachhead toddler–preschool). Parents should never need another app: Hale lives in SMS and email, and **the parent opens the thread — Hale never texts first**, which is the promise the /text page makes in those words and the posture CASL is read against here. Today Hale recommends and prepares; completing a registration itself is roadmap (VIL-338 / VIL-339, Stripe plus execute) and is deliberately **not** part of the pitch. The web app is the receipts surface (approvals, audit, settings) rather than the daily one. The F14 decision register (D1–D25) fixes the choices that rest on: messaging-first surface, single-ask intake then just-in-time everything, the registration-and-coverage radar as the flagship job, scoped caregiver roles, privacy as the moat, natural-language consent, ICS-first calendar output, the dashboard demoted to receipts, and — D22 to D25 — the positioning above, asking only about what Hale actually saw, reviews and attendance as byproducts rather than a ratings product, and never-template meaning voice rather than generation. **Release is no longer one flag flip.** There are eight or more independent dark flags, each with its own probe and a staged lighting order; the current list and that order live in the Linear document "Product direction 2026-09-21". The [F14 · Messaging-First Operator](https://linear.app/villagehale/project/f14-messaging-first-operator-hale-is-a-number-you-text-ad65e88d5775) Linear project — its design documents, that register, and the per-feature briefs — is the design source of truth; read it before changing product behaviour, and treat the pre-pivot newborn-platform spec, the "family chief of staff" framing and the six 2026-07 design documents as history rather than direction (each of those six is annotated in Linear with what it now gets wrong). (Compliance baseline is Canada — PIPEDA/Law 25 + data residency per hard rule #1; broadening to other regions is a deliberate multi-region decision, not an assumption.)
