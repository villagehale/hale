...
**Status:** approved 2026-09-20. This is the single source of truth for the register every parent-facing message is written in, the strings that may never change, and the boundaries this register does not reach. Its runtime half is `packages/agent/skills/voice-register.md`, a frontmatter-less partial that a composing skill pulls in with `{{include:voice-register}}` — **ten of them do today, eight refused it on measurement and five have not been offered it yet; the list and the numbers are below**. The deterministic copy files are held to the same rules by their own tests, and on a skill that refused the partial the rules still reach the model, in that skill's own words.
...
**Which skills read the partial, and which read the rules in their own words.** The include is not a style preference, it is a change to a system prompt, and every one of these lanes has an eval suite that grades the message a parent would have received. The rule this list was built by is the one the suites enforce: *a skill takes the include if and only if its own corpus still passes with it in place.* Ten do.

| | skills | how it was decided |
|---|---|---|
| **read the partial** | `calendar-email-ask`, `calendar-invite-note`, `followup-voice`, `identity-ask`, `intake-voice`, `intro-voice`, `nudge-voice`, `reminder-voice`, `week-summary`, `welcome-voice` | eight suites re-run live with the include in place, every gate held, caches committed |
| **keep their own copy** | `coach-channel-sms`, `radar-voice`, `coach-plan`, `coach-plan-note`, `general-answer`, `intake-answer`, `turn-apology`, `voice-turn` | each suite went red on a live re-run; the numbers are below |
| **not offered it yet** | `activity-finder`, `activity-deep`, `activity-synthesis`, `medical-symptom`, `ask-hale` | the finder lanes, PR6c. `activity-synthesis` has a runner (`apps/worker/evals/run-activity-synthesis-eval.mjs`) but no `eval:*` script and no CI step, so a live re-run of it would not be a gate anyone re-runs; it cannot be offered the include until it is wired |

**The marker goes at the END of a skill file**, after the lane's own boundaries, and that is measured rather than tidy. Put at the top, under the H1 where the coach's brief placed it, 4,792 characters of house register sit between the job and the data — and on `identity-ask` two of six asks came back over the 69-character tail budget, which is `UNSENDABLE ASKS`: the parent is never asked at all. The same corpus with the marker at the end: zero. `reminder-voice`, `intake-voice` and `nudge` moved from red to green the same way. A house rule that costs a lane its own ceiling is a regression wearing a consistency badge.

**What a refusal looked like, so the next reader does not re-litigate it from taste.** The eight refusals are one live run each, against the real skill through the real loader:

- `turn-apology` — the loudest. Its gate draws the same prompt eight times and counts how many come back different, because the skill exists to end the era when Hale sent one fixed sentence to every parent whose turn broke. With the register appended, seven of the eight came back as the identical string (`"That one broke on my end and nothing changed."`): **2 distinct of 8, against a floor of 5**. The section immediately above the include is titled *"No examples, deliberately"*.
- `voice-turn` — a recall miss (the turn never said the thing it had just looked up) and a fixture at voice 2; corpus mean 4.91 → 4.36.
- `general-answer` — four fixtures below the judge floor. `intake-answer` — two. `coach-plan` / `coach-plan-note` — one.
- `radar-voice` — two fixtures came back as *"answer failed to parse into a strict { message } object"*: no message at all, on the first useful sentence Hale ever sends a family. It is the largest of these skills, and the partial put 4.8 KB between the model and its output contract.
- `coach-channel-sms` — refused on prompt budget before a corpus ever ran; the paragraph below has the arithmetic.

**These are not judge tails, and the control says so.** Every one of these suites takes a single judge draw, so a fresh live run can go red on sampling alone. Two checks separate the two explanations. First, the direct control: `identity-ask` re-run live with its committed text unchanged and only one semantically empty blank line added — enough to re-key the cache, not enough to change a word of the prompt — passed clean (0 unsendable, 0 below the floor, 4 distinct opens), while the same corpus with the include failed a deterministic character gate twice. Second, the base rate: the same append, at the same placement, was green on 8 suites of 8 in the proactive cluster and 1 of 6 in the conversational one. Sampling noise does not sort itself by cluster. And three of the reds — `turn-apology`'s distinct count, `voice-turn`'s recall miss, `radar-voice`'s parse failures — are not judged at all.

**What this says about the partial itself, which is a decision for this file and not for a skill PR.** The register partial is 4,792 characters and carries two different things: the invariant rules, and worked phrasings of them (*"how did swim go?"*, *"Tue 7:00 a.m."*). The rules are what a lane needs. The phrasings are a style attractor, and the numbers say so even where a suite passed — `calendar-voice`'s note bodies went from 0.57 to 0.71 most-similar-pair against a 0.75 gate, `intro-voice` 0.57 → 0.66, `identity-ask` from 4 distinct opens to 2 against a floor of 2, `intake-voice`'s shipped mean from 4.63 to 3.63 against a floor of 3. Every one of those still passes; several now sit on their floor. A register partial of a few hundred characters — the bans and the one-line register, no sample sentences — would plausibly be taken by all twenty-three lanes and would buy the agreement without the collapse. Shrinking it is a change to the register, so it belongs to this document and to whoever owns it, not to the next skill edit.

**The coach skill keeps its own copy of these rules — the rules reach it, the PARTIAL does not.** Measured through the real `parseSkill` with the include in place: it lets `packages/agent/skills/coach-channel-sms.md` release 573 characters of its own text (the plain-ASCII bullet, the sentence-case clause and the how-to-end clause, all three carried by the partial) and adds back 4,792, so the prompt the model actually reads goes from 41,046 to 45,239 characters — **+10.2%**. The condition on that include was a net subtraction, and on the one lane whose output budget is 400 tokens with thinking sharing it — where an eleventh tool already cost tool reach on the two fixtures that most needed it (VIL-294) — 10% more system prompt is not one. So the coach states this register in its own words and its own corpus gate holds it there: `apps/worker/evals/coach-channel-voice-tells.mjs` fails the whole corpus on the register's negative rules, which is a stronger guarantee than the include would have bought. The include is not "deferred": it is out until the partial itself is small enough to pay for it — at most 573 characters against 4,792 today — and shrinking the partial is a change to the register, which is a decision about this file rather than a skill edit.
