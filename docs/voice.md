...
| **not offered it yet** | `activity-finder`, `activity-deep`, `activity-synthesis`, `medical-symptom`, `ask-hale` | the finder lanes, PR6c. `activity-synthesis` has no eval suite at all, so it cannot be offered the include until it has one |
