...
// ─── Memory graph: longitudinal family-context write/read path ────────────

/** The fact_type enum from the schema — keeps callers honest about the closed set. */
type MemoryFactType = schema.NewFamilyMemoryFact['factType'];

/** How many most-recent episodes a memory slice carries to an agent. Bounded so
 * the slice that goes into a drafter/inferencer prompt stays small. */
const MEMORY_SLICE_EPISODE_LIMIT = 20;
...
/**
 * Upserts one family-memory fact with valid_until supersedence: if a currently-
 * valid row exists for (family_id, fact_type, fact_key), it is invalidated
 * (valid_until = now, superseded_by = new id) and a fresh row is inserted as the
 * new currently-valid version. Old versions are never deleted — the history is
 * the moat. The supersede + insert + audit row all commit in ONE transaction so
 * a (family, type, key) never has two valid rows and the audit trail (hard rule
 * #6) can never be separated from the write.
 */
export async function upsertMemoryFact(
  input: UpsertMemoryFactInput,
  database: Database = db(),
): Promise<{ factId: string; superseded: boolean }> {
  return recordTransition<{ factId: string; superseded: boolean }>(async (tx) => {
    const inserted = await tx
    ...
    if (!factId) {
      throw new Error('family_memory_facts insert returned no row');
    }

    // Invalidate any pre-existing valid version, pointing it at the row we just
    // wrote. Excluding the new row's own id keeps the insert→supersede order
    // from invalidating itself.
    const superseded = await tx
      .update(schema.familyMemoryFacts)
      .set({ validUntil: new Date(), supersededBy: factId })
      .where(
        and(
          ...
          sql`${schema.familyMemoryFacts.id} <> ${factId}`,
        ),
      )
      .returning({ id: schema.familyMemoryFacts.id });

    return {
      value: { factId, superseded: superseded.length > 0 },
      audit: {
        familyId: input.familyId,
        actor: 'system',
        actionTaken: 'memory.fact.upserted',
...
interface AppendMemoryEpisodeInput {
  familyId: string;
  childId?: string;
  occurredAt: Date;
  episodeType: string;
...
/**
 * Appends one episodic-memory row and its audit row in one transaction. Episodes
 * are append-only — they are the timeline the inferencer and coach scan.
 */
export async function appendMemoryEpisode(
  input: AppendMemoryEpisodeInput,
  database: Database = db(),
): Promise<{ episodeId: string }> {
  return recordTransition<{ episodeId: string }>(async (tx) => {
    const inserted = await tx
    ...
    if (!episodeId) {
      throw new Error('family_memory_episodes insert returned no row');
    }
    ...
    return {
      ...
      audit: {
        familyId: input.familyId,
        actor: 'system',
        actionTaken: 'memory.episode.appended',
        targetTable: 'family_memory_episodes',
        targetId: episodeId,
        after: { episodeType: input.episodeType, occurredAt: input.occurredAt.toISOString() },
      },
    };
  }, database);
}
...
/**
 * Retires a fact by invalidating its currently-valid row (valid_until = now),
 * without writing a replacement — the inferencer's `retire_facts` path. A
 * fact_key with no currently-valid row is a no-op: nothing changed, so no audit
 * row is warranted (mirrors recordDiscovery's empty-input policy).
 */
export async function retireMemoryFact(
  input: { familyId: string; factType: MemoryFactType; factKey: string },
  database: Database = db(),
): Promise<{ retired: boolean }> {
  // Probe outside the transition so a no-op writes nothing (no audit row).
  ...
  await recordTransition(async (tx) => {
    await tx
      ...
      .where(eq(schema.familyMemoryFacts.id, validId));
    ...
    return {
      ...
      audit: {
        familyId: input.familyId,
        actor: 'system',
        actionTaken: 'memory.fact.retired',
        targetTable: 'family_memory_facts',
        targetId: validId,
        after: { factType: input.factType, factKey: input.factKey },
      },
    };
  }, database);
...
export interface MemorySlice {
  facts: schema.FamilyMemoryFact[];
  episodes: schema.FamilyMemoryEpisode[];
}

/**
 * Reads a family's current memory: every currently-valid fact (valid_until IS
 * NULL — the lookup index is partial on exactly this predicate) plus the most
 * recent episodes. This is the snapshot the drafter consults for context and the
 * inferencer diffs against. An empty family returns {facts:[],episodes:[]} — the
 * callers skip gracefully rather than null-masking.
 */
export async function getMemorySlice(
  familyId: string,
  database: Database = db(),
): Promise<MemorySlice> {
  ...
  const episodes = await database
    ...
    .orderBy(sql`${schema.familyMemoryEpisodes.occurredAt} DESC`)
    .limit(MEMORY_SLICE_EPISODE_LIMIT);

  return { facts, episodes };
}

/** The assembled input the Memory Inferencer consumes. Structurally what the
 * agent's InferencerJob expects; defined here (not imported from the agent) so
 * the service layer stays a leaf the agent depends on, not vice-versa. */
export interface InferencerJobInput {
  familyId: string;
  windowDays: number;
  recentEvents: schema.Event[];
  recentActions: schema.Action[];
  currentMemorySnapshot: MemorySlice;
}

/**
 * Assembles the Memory Inferencer's job from the DB: the family's events and
 * actions within the window, plus the current memory snapshot it diffs against.
 * The scheduled `memory.inference.due` job carries only {familyId, windowDays};
 * this turns that into the real input — no dummy data. Events/actions are bounded
 * to the window so a nightly run sees the recent burst, not the whole history
 * (the longitudinal view lives in the snapshot's facts/episodes).
 */
export async function loadInferencerJob(
  input: { familyId: string; windowDays: number },
  database: Database = db(),
): Promise<InferencerJobInput> {
  const since = sql`now() - make_interval(days => ${input.windowDays})`;

  const recentEvents = await database
    .select()
    .from(schema.events)
    .where(and(eq(schema.events.familyId, input.familyId), sql`${schema.events.receivedAt} >= ${since}`))
    .orderBy(sql`${schema.events.receivedAt} DESC`);

  const recentActions = await database
    .select()
    .from(schema.actions)
    .where(and(eq(schema.actions.familyId, input.familyId), sql`${schema.actions.draftedAt} >= ${since}`))
    .orderBy(sql`${schema.actions.draftedAt} DESC`);

  const currentMemorySnapshot = await getMemorySlice(input.familyId, database);
