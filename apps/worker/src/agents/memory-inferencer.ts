 ...
 * HARU_NOT_CONFIGURED rather than silently parse output and drop it —
 * a "succeeded" run with no DB writes is the dishonest stub pattern.
 *
 * To enable: implement upsert helpers in @haru/db for
...
export async function runMemoryInferencer(job: InferencerJob): Promise<void> {
  const err = new Error(
    `HARU_NOT_CONFIGURED: Memory Inferencer is disabled until @haru/db exposes family_memory_facts upsert + family_memory_episodes append (familyId=${job.familyId}, windowDays=${job.windowDays})`,
  );
