...
describe('landing — sections, in the Surfaces Plan order', () => {
  ...
  it('tells the three texts and the suggest-and-prepare line with receipts', () => {
    // VIL-325: first text is names/ages/postal, not "hi". "no menus" stays;
    // "no forms" does not. #684 locked the consent line: Hale asks how it went
    // and does not book.
...
describe('landing — structured data', () => {
  it('emits its own JSON-LD graph describing the page a visitor actually sees', () => {
    ...
    // #684 locked the graph to the kids-year category. The old "a number you text"
    // sentence is the one this graph used to sell.
