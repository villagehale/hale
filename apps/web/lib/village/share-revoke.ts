 ...
 *
 * One token-bearing table carries a shareable link:
 *   - village_candidates.share_token → one endorsed activity card (/a).
 * (The week-plan/picks token died with routine_proposals — F13's leg.)
 */

export type ShareLinkKind = 'activity';

export interface SharedLink {
  kind: ShareLinkKind;
  /** The owning row's id (village_candidates.id). */
  id: string;
  token: string;
  /** A human label for the list — the activity's title. */
  title: string;
}
...
export async function revokeShareLink(
  ...
  const { id, familyId, actorUserId } = args;
