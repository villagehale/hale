...
// The route reads the Auth.js session + db + the Concierge agent at request time.
...
vi.mock('~/lib/coach/agent', () => ({ runConcierge: (...a: unknown[]) => runConciergeMock(...a) }));
...
describe('POST /api/coach — auth + spend gating', () => {
  ...
  it('streams delta events then a terminal done with conversationId + action intents', async () => {
    ...
    runConciergeMock.mockImplementation(
      async (
        _input: unknown,
        _db: unknown,
        _client: unknown,
    ...
    expect(runConciergeMock).toHaveBeenCalledWith(
      {
        familyId: 'fam-1',
        question: 'my teen barely talks to me anymore',
        intent: null,
  ...
  it('emits a terminal error event when the agent run fails', async () => {
    configureAuth(true);
    authMock.mockResolvedValue(session('google-1'));
    runConciergeMock.mockRejectedValue(new Error('agent blew up'));
